...
class EpochEndMAP(Callback):
    def __init__(self, func, gen, labels):
        self.func = func
        self.gen = gen
        self.labels = labels
        # print("generator map", gen, self.gen)

    def on_epoch_end(self, epoch, logs={}):
        average_precisions = self.func(self.gen)

        out = 'Validation mAP: {:.4f}\n'.format(sum(average_precisions.values()) / len(average_precisions))
...
class EpochEndMAPK(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        # print(self.valid_gen.size())
        all_precisions = []
        for i in range(self.valid_gen.size()):
            raw_image = self.valid_gen.load_image(i)
            annotations = self.valid_gen.load_annotation(i)
            gt_boxes = []
            for a in annotations:
                if len(a) > 0:
                    gt_boxes.append([int(a[0]), int(a[1]), int(a[2] - a[0]), int(a[3] - a[1])])

            # print(gt_boxes)

            boxes = self.predict(raw_image, self.model)
            scores = np.array([box.c for box in boxes])
            pred_boxes = np.array([[int(box.xmin * 1024), int(box.ymin * 1024), int((box.xmax - box.xmin) * 1024), \
                                    int((box.ymax - box.ymin) * 1024)] for box in boxes])
        ...
        out = "The average precision over validation images: {0:.4f}\n".format(np.mean(all_precisions))
        f = open('metrics.txt', 'a')
        f.write(out)
        f.close()
        print(out)
...
class YOLO(object):
    def __init__(self, backend,
        ...
        print("==========================================")
        print()
        print(self.nb_box, self.nb_class, self.true_boxes)
    ...
    def train(self, train_imgs,  # the list of images to train the model
        ...
        # Compile the model
        ############################################
        # self.model.load_weights("full_yolo_raccoon.h5")
        ...
        evaluation = EpochEndMAP(self.evaluate, valid_generator, self.labels)
        score = EpochEndMAPK(valid_generator)
        ...
        self.model.fit_generator(generator=train_generator,
                                 ...
                                 callbacks=[early_stop, checkpoint, evaluation, score],
                                 workers=6,
                                 max_queue_size=8)
    ...
    def evaluate(self,
        ...
        for i in range(generator.size()):
            ...
            # print(i,len(annotations[0]), annotations[0])
    ...
    def predict(self, image, obj=0.3, nms=0.3):
        image_h, image_w, _ = image.shape
        ...
        boxes = decode_netout(netout, self.anchors, self.nb_class, obj, nms)
